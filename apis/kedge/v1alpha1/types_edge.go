...
// Edge annotation keys.
const (
	// EdgeAnnotationRegenerateJoinToken, when set to any non-empty value on an
	// Edge, instructs the token reconciler to mint a fresh bootstrap join token
	// (clearing status.joinToken first if needed) and reset the Registered
	// condition to False. The annotation is removed once the new token is
	// issued. Used to recover edges whose original token was consumed and whose
	// agent has lost its durable credential.
	EdgeAnnotationRegenerateJoinToken = "kedge.faros.sh/regenerate-join-token"
)
...
type EdgeStatus struct {
	...
	// path to correlate. Stamped by the hub's edge controller, which resolves
	// it from the tenant LogicalCluster's kcp.io/path annotation — something a
	// provider service account cannot read itself. Consumed by the kuery
	// engagement controller to label engaged clusters so tenant queries scope
	// correctly.
	// +optional
	...
	// LastHeartbeatTime is the timestamp of the most recent heartbeat the agent
	// sent to the hub. Stamped agent-side, so accuracy depends on the agent's
	// clock; useful as a liveness signal. Heartbeats arrive every ~30s; the hub
	// marks an edge disconnected after ~90s without one.
	// +optional
