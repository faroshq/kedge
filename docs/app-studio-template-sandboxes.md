...
| **2** | App Studio: `Project.spec.template`, creation wizard + assistant interview, component-aware sync/logs/restart, template-aware assistant context. `defaultSandboxRunnerBinding` replaced. | Medium |
| **3** | Code provider checkout capability; repo import flow; template switching (§4.3) end to end. | Medium |
| **4** | Retire the `sandbox-runner` Template (its use case = a static/web template in dev mode). **Existing projects are auto-migrated**: commit workspace → rewrite the binding to the equivalent web template with `kedgeMode: development` → re-provision → re-hydrate (the §4.3 switch flow, run by the platform). Docs; BYO-compute validation with a dev-mode template on a second infra provider. | Low |
