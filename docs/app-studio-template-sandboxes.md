...
| **4** | Retire the `sandbox-runner` Template (its use case = a static/web template in dev mode) and delete the legacy code paths outright: `defaultSandboxRunnerBinding`, the flat data-plane verbs, the signed sandbox-gateway preview flow, and the sandbox-runner template/image. **No migration** — decided 2026-07-04: pre-GA, no backward compatibility owed; existing sandbox-runner projects are simply deleted/recreated (their code lives in git). Docs; BYO-compute validation with a dev-mode template on a second infra provider. | Low |
