...
```
http://localhost:8080/apis/clusters/<cluster-id>
```

Example: `http://localhost:8080/apis/clusters/a08bvxnomis2m558`

Other endpoints:

| Path | Description |
|------|-------------|
| `GET /apis/clusters/{cluster}` | GraphQL playground UI |
| `POST /apis/clusters/{cluster}` | GraphQL query endpoint |
...
```bash
curl -X POST http://localhost:8080/apis/clusters/<cluster-id> \
...
```bash
kubectl create --raw '/graphql/apis/clusters/<cluster-id>' -f - <<'EOF'
...
```bash
kubectl create --raw '/graphql/apis/clusters/a08bvxnomis2m558' -f - <<'EOF'
{"query":"{ v1 { Namespaces { items { metadata { name } } } } }"}
EOF
```
...
```bash
kubectl get --raw '/graphql/apis/clusters/a08bvxnomis2m558'
