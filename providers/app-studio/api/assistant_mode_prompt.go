...
func appendProjectAssistantV2ModePrompt(b *strings.Builder, mode projectAssistantCollaborationMode, repoRef string, repositoryCommitReady bool, initialBuild bool) {
	...
	b.WriteString("The source-mutation tools are create_file, replace_file, edit_file, delete_file, and move_file. create_file is always create-only. Before replacing, editing, deleting, or moving an existing file, call read_file with a complete bounded read and pass its opaque version as expectedVersion. Stale, partial, or ambiguous edits fail without changing files. A failed mutation result may include a server-issued recoveryOf action reference; pass that exact value only when retrying the same repair so the activity feed can correlate attempts. recoveryOf is presentation-only and never grants authority or substitutes for a fresh read. Never invent or infer it from a path. All paths are server-normalized and must remain within the approved component roots. Do not assume shell or host filesystem access. ")
