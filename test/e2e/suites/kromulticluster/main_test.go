...
// Package kromulticluster e2e-tests the tenant isolation guarantees of
// the kro-multicluster provider's REST surface. We start ONLY the
...
func TestMain(m *testing.M) {
	...
	if portInUse(providerPort) {
		fmt.Fprintf(os.Stderr, "port :%s in use; pkill kromulticluster-provider and retry\n", providerPort)
		os.Exit(2)
	}
	...
	dataDir, err := os.MkdirTemp("", "kedge-e2e-kromc-")
	if err != nil {
	...
	provCmd := exec.Command(filepath.Join(repoRoot, "bin", "kromulticluster-provider"))
	...
	fmt.Fprintf(os.Stderr, "kromulticluster-provider started (pid=%d, port=:%s, log=%s)\n",
...
func build(root string) error {
	cmd := exec.Command("make", "-C", root, "build-kromulticluster-provider")
